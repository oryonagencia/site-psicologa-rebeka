...
const filePath = "d:/Documentos/Oryon Agência/Site com IA/Site-psicologa-rebeka/dist/index.html";
...
// 4. Add aria-hidden to all SVGs
content = content.replaceAll('<svg ', '<svg aria-hidden="true" ');

// 5. Fix FAQ ARIA attributes
for (let i = 1; i <= 8; i++) {
    content = content.replace(
        `<button @click="active !== ${i} ? active = ${i} : active = null" class="w-full text-left px-8 py-6 bg-white hover:bg-fundo/50 focus:outline-none flex justify-between items-center transition-colors">`,
        `<button @click="active !== ${i} ? active = ${i} : active = null" :aria-expanded="active === ${i} ? 'true' : 'false'" aria-controls="faq-answer-${i}" class="w-full text-left px-8 py-6 bg-white hover:bg-fundo/50 focus:outline-none flex justify-between items-center transition-colors">`
    );
    content = content.replace(
        `<div x-show="active === ${i}" x-collapse style="display: none;" class="px-8 pb-6 bg-white text-texto text-lg border-t border-gray-50 pt-4">`,
        `<div id="faq-answer-${i}" x-show="active === ${i}" x-collapse style="display: none;" class="px-8 pb-6 bg-white text-texto text-lg border-t border-gray-50 pt-4">`
    );
}

fs.writeFileSync(filePath, content, 'utf-8');
console.log("SEO & A11y Fixes applied successfully.");
